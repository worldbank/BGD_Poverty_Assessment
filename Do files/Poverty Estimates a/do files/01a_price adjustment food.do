...
*****************************************************************************************************/
	...
	duplicates report hhid day item
	duplicates tag hhid day item, gen(tag)
	duplicates drop hhid day item, force
	sort hhid item day
	...
	*There are 5 quantities with negative sign, we change the sign to positive
	...
	replace quantity=1    if hhid==231054 & day==3  & item==192  
	replace quantity=2    if hhid==1619030 & day==9 & item==192  
    replace quantity=1500 if hhid==1654086 & day==1 & item==12
    replace quantity=10   if hhid==1710061 & day==13 & item==204
    replace quantity=1500 if hhid==2125053 & day==6 & item==13
	...
	*2) Count number of observations without outliers
	bysort hhid      item: egen counthhold= count(p)
	bysort psu        item: egen countpsu=  count(p)
	bysort zila_code  item: egen countzila= count(p)
	...
	*A- Calculate median by household and item
	bysort hhid      item: egen medianhhold=   median(p)
	...
	*C-calculate median by zila	and item
	 qui levelsof zila_code, local(strat)
	 qui levelsof item, local(food)
 	  qui gen medianzila = . 
	  foreach z of local strat {
	           foreach f of local food {
		qui su p [aw = hhwgt] if zila_code == `z' & item==`f', detail
		qui replace medianzila = r(p50) if zila_code == `z' & item==`f' & medianzila == .
        }
	    }
   ...
   *Impute the total value if quantity>0 and value is zero or missing 
    ...
    *****************************************************************************************************/
    ...
    duplicates report hhid week item
    duplicates drop hhid week item, force
	...
	*There are 3 quantities with negative sign, we change the sign to positive
	...
	replace quantity=75     if hhid==1695126 & week==1  & item==211
	replace quantity=125    if hhid==1710018 & week==2  & item==214 
    replace quantity=50     if hhid==1710061 & week==2  & item==215
	...
	*C-calculate median by zila	and item
	...
	bysort hhid    item: egen countoutl= count(p) if p==.
    tab countoutl	  
