...
******************************************************************************************************
*****************************************************************************************************/
/*This do file computes 2016 annual budget shares and  prices to calculate a torqvist index. 
The upper and lower poverty lines 2016 are created updating the poverty lines from 
2010 with a composite price index. */
/*****************************************************************************************************
*                                                                                                    *
                                   INITIAL COMMANDS
*                                                                                                    *
...
*****************************************************************************************************/
	...
	*This duplicates acts as a collapse, it drops lots of observations to convert roster into hhold level dataset
	...
	save "$temporary/memberall", replace
	...
	*drop if member==.
	drop _merge  
	sort hhold	
	save "$temporary/temp0all", replace
	...
	*Daily consumption, count days 
	...
	collapse (sum) fexpday=value, by(psu hhid hhid day)
	drop if (fexpday==0 | fexpday==.)
	*Remove those days for which no consumption is reported
	collapse (sum) fexptot=fexpday (count) ndays=day, by(psu hhid hhid)
	sort hhid
	save "$temporary/ndaysall", replace
	...
	*Group daily food items into 12 main categories 
	...
	collapse (sum) v=value, by(psu hhid hhid itemcode)
	drop if itemcode==.
	sort hhid

	merge m:1 hhid using "$temporary/ndaysall" 
	...
	*Convert daily consumption to monthly consumption (factor is 365/12)
	...
	drop ndays
	reshape wide v, i(psu hhid hhid) j(itemcode)
	save "$temporary/temp1all", replace
	...
	* Weekly consumption
	...
	collapse (sum) fexpwk=value, by(psu hhid hhid week)
	drop if (fexpwk==0 | fexpwk==.)
	collapse (sum) fexptot2=fexpwk (count) nweeks=week, by(psu hhid hhid)
	sort hhid
	save "$temporary/temp1aall", replace
	...
	sort hhid
	merge hhid using "$temporary/temp1aall"
	tab _m
	drop _m
	*Convert weekly to monthly expenditure
	...
	reshape wide v, i(psu hhid hhid) j(itemcode)
	save "$temporary/temp1ball", replace

	use "$temporary/temp1all", clear
	merge 1:1 hhid using "$temporary/temp1ball"
	tab _m
	drop _m
	...
	*Merge with consumption expenditure database
	merge 1:1 hhid using "$temporary/temp0all"
	tab _merge
	
	*we drop households when is not possible to calculate v1-v13
	drop if _m==2
	keep psu hhid hhid stratum16 hhwgt v1-v13 cons_exp 
	order psu hhid hhid stratum16 hhwgt v1 v2 v3 v4 v5 v6 v7 v8 v9 v10 v11 v12 v13
	
	*Generate budget shares for each hhid
	...
	*Weighted Avg. of budget shares, by stratum
	...
	save "$temporary/budg2016all.dta", replace
...
* ----------------------------------------------------------------------
	...
	*gen p = (value/quantity) 	
	...
	collapse (mean) p, by(psu hhid hhid itemcode)
	sort hhid
	merge hhid using "$temporary/temp0all"
	tab _merge
    keep if _merge==3
	drop _merge
	keep psu hhid hhid itemcode stratum16 p hhwgt 
	save "$temporary/temp2all", replace
	...
	*gen p = (value/quantity)
	...
	drop _merge
	keep psu hhid hhid  itemcode stratum16  p hhwgt 
	append using "$temporary/temp2all"
	...
	* Prepared betel leaves, 
	...
	sort stratum16
	save "$temporary/price2016all", replace
...
*****************************************************************************************************/
	...
	use "$input2010/budgsh10_new.dta"
	ren stratum stratum16
	tempfile hies2010
	save `hies2010', replace
	
	use "$input2010/price10_new.dta",clear
	ren stratum stratum16
	merge 1:1 stratum16 using `hies2010'
	tab _m
	drop _m
	sort stratum16
	merge 1:1 stratum16 using "$temporary/price2016all"
	tab _m
	drop _m
	sort stratum16
	merge 1:1 stratum16 using "$temporary/budg2016all.dta"
	drop _m
	...
	*index
	...
	save "$temporary/priceindex16_10_hies2016all", replace
	...
	*Create non-food CPI
	gen     nfcpi=1.530  if urbrural==1
	replace nfcpi=1.553  if urbrural==2
	...
	*Merge with 2010 poverty lines
	merge 1:1 stratum16 using "$input2010/povline10"
	drop _m
	
	*Annual poverty lines are created updating the lower and upper poverty lines from 2010
	gen zf16=zf10*index
	gen zu16=zu10*index
	gen zl16=zl10*index
	...
	la var zf16  "Food Poverty line 2016"
	la var zl16  "Lower poverty line 2016"
	la var zu16  "Upper poverty line 2016"
	
	save "$output/povline16all", replace

	
	*Erase temporary files
	erase "$temporary/ndaysall.dta"
	erase "$temporary/temp0all.dta"
	erase "$temporary/temp1all.dta"
	erase "$temporary/temp1aall.dta"
	erase "$temporary/temp1ball.dta"
	erase "$temporary/temp2all.dta"
