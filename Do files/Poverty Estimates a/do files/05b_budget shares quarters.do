...
*****************************************************************************************************/
	...
	*Duplicated observations
	duplicates report hhid s1aq00
	duplicates tag hhid s1aq00, gen(flag)
	*Drops automatically one of the duplicated observations
	sort hhid s1aq00
	duplicates drop hhid s1aq00, force
	egen member= count(s1aq00), by(hhid)
	drop if member==0
	*This duplicates acts as a collapse, it drops lots of observations to convert roster into hhid level dataset
	sort hhid
	duplicates drop hhid, force
	keep psu hhid hhid member stratum16 div ruc zilaid zila zila_name
	...
	* To calculate budget shares, HIES traditionally uses consexp variable (i.e. consumption before adjusting for rent)
	...
	keep psu hhid hhid stratum16 hhwgt cons_exp urbrural term
	merge 1:1 hhid using "$temporary/member`i'"
	tab _merge
	drop if _merge==2
	*drop if member==.
	drop _merge  
	...
	*Group daily food items into 12 main categories 
	...
	merge m:1 hhid using "$temporary/ndays`i'"
	tab _m
    keep if _m==3
	drop _m
	...
	* Weekly consumption
	...
	sort hhid
	merge hhid using "$temporary/temp1a`i'"
	tab _m
	drop _m
	*Convert weekly to monthly expenditure
	...
	merge 1:1 hhid using "$temporary/temp1b`i'"
	tab _m
	drop _m
	...
	*Merge with consumption expenditure database
	merge 1:1 hhid using "$temporary/temp0`i'"
	tab _merge
...
* ----------------------------------------------------------------------
	...
	*gen p = (value/quantity) 	
	...
	sort hhid
	merge hhid using "$temporary/temp0`i'"
	tab _merge
    keep if _merge==3
	drop _merge
