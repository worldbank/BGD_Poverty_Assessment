...
collapse (sum) wage_inc_cash=wage_inc_cash wage_inc_kind=wage_inc_kind wage_inc=wage_inc (max)div zila ruc,by(hhid)
...
replace s5q13=4992000 if hhid==2013017
...
collapse (sum) gr_buss_inc=gr_buss_inc buss_expenses=buss_expenses net_buss_inc=net_buss_inc  s5q20=s5q20 ,by(hhid)
sort  hhid
...
bys hhid: egen total=sum(crop_inc)

collapse (sum) crop_inc=crop_inc,by(hhid)
sort  hhid
...
collapse (sum) live_inc1=live_inc1,by(hhid)
sort  hhid
...
collapse (sum) live_inc2=live_inc2,by(hhid)
sort hhid
...
collapse (sum) fish_inc=fish_inc,by(hhid)
sort  hhid
...
collapse (sum)  prop_inc=prop_inc remittance_inc=remittance_inc social_inc=social_inc othnonwage_inc=othnonwage_inc,by(hhid)
sort hhid
...
keep psu hhid div zila wgt post_factor ruc s8cq00 s8cq06 s8cq14 s8cq17
...
collapse (sum)  tot_remit= tot_remit tot_remit_abr=tot_remit_abr tot_remit_dom=tot_remit_dom (max) urbrural,by(hhid)
sort hhid
...
use "$input\HH_SEC_8B_Q1Q2Q3Q4",clear
keep  hhid s8bq08 s8bq09 s8bq10
...
collapse (sum)  snet_cash=snet_cash snet_kind=snet_kind snet_cash_last=snet_cash_last snet_kind_last=snet_kind_last,by(hhid)
...
collapse (sum)  stipend_inc=stipend_inc,by(hhid)
sort hhid
...
collapse (sum) imprent=imprent  (max) s6aq23,by(hhid)
sort hhid
...
merge 1:1 hhid using business_income
tab _m
drop _m
sort  hhid
merge  1:1 hhid using crop_income
tab _m
drop _m
sort  hhid
merge 1:1 hhid using live_income1
tab _m
drop _m
sort  hhid
merge  1:1 hhid using live_income2
tab _m
drop _m
sort  hhid
merge  1:1 hhid using fish_income
tab _m
drop _m
sort  hhid
merge  1:1 hhid using forest_income
tab _m
drop _m
sort  hhid
merge  1:1 hhid using agri_asset_income
tab _m
drop _m
sort  hhid
merge 1:1 hhid using oth_income
tab _m
drop _m
sort  hhid
merge 1:1 hhid using remittances
tab _m
drop _m
sort  hhid

merge 1:1 hhid using safetynet_income
tab _m
drop _m
sort hhid
merge 1:1 hhid using stipend_income
tab _m
drop _m
sort  hhid
merge 1:1 hhid using imputed_income
tab _m
drop if _m==2
drop _m
sort hhid
