...
*****************************************************************************************************/
	...
	** Daily consumption**
	...
	collapse (sum) fexpday=value (mean) stratum16, by(psu hhid hhold day)

	*Remove those days for which no consumption is reported
	drop if (fexpday==0 | fexpday==.)

	*Make 14 days food consumption
	collapse (sum) fexptot1=fexpday (count) ndays=day (mean) stratum16, by(psu hhid hhold)
	...
	** Weekly consumption**
	...
	drop if mod(item,10)==0
	collapse (sum) fexpwk=value (mean) stratum16, by(psu hhid hhold week)

	*To remove those days for which no consumption is reported
	drop if fexpwk==0

	*Make 14 days(2 weeks) food consumption
	collapse (sum) fexptot2=fexpwk (count) nweeks=week (mean) stratum16, by(psu hhid hhold)
	...
	*Make monthly food consumption
	...
	merge 1:1 hhold using "$temporary/temp2"
	tab _merge
...
*****************************************************************************************************/
	...
	*There are not repeated observations	
	...
	drop if mod(item,10)==0
	collapse (sum) nfood1=value (mean) stratum16, by(psu hhid hhold)
	sort hhold
	save "$temporary/temp1", replace
	...
	* Section 9, Part D (items 311 - 362)
	...
	collapse (sum) nfood2=value (mean) stratum16, by(psu hhid hhold)
	...
	*drop insurance
	...
	collapse (sum) nfood3=value hlth_exp edu_exp oth_exp, by(psu hhid hhold)
    ...
    sort hhold
    merge 1:1 hhold using "$temporary\temp3"
    tab _m
    keep if _m==3
    drop _m
    ...
    merge 1:1 hhold using "$temporary\temp2"
    tab  _merge
    drop _merge
    sort hhold
    merge hhold using "$temporary\temp4"
    tab  _merge
    drop _merge
    sort hhold
...
*****************************************************************************************************/
	...
	sort hhold
	merge 1:1 hhold using "$temporary/nfexp_hies2016"
	tab  _merge
	drop _merge
	egen hhexp = rowtotal(fexp nfexp)

	keep psu hhid hhold fexp nfexp hhexp 
