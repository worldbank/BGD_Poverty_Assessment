...
*****************************************************************************************************/
	...
	*There are not repeated observations	
	...
	collapse (sum) lumpyexp=value, by(psu hhid hhold)
	...
	*Merge with lumpy expenditure
	merge 1:1 hhold using "$temporary/lumpyexpenditure"
	tab _m
	drop if _m==2
	drop _m
	...
	*Add geographical variables
	merge 1:1 hhold using "$input/HH_SEC_A_Q1Q2Q3Q4.dta", keepusing(id_01_name zl id_03_code id_03_name id_04_code id_04_name id_05_code id_05_name) 
	keep if _m==3
	drop _m	
...
merge 1:1 hhold using "$output\household_income2016", generate(merge_inc) keepusing(hhincome) 
tab merge_inc
drop if  merge_inc==2
drop  merge_inc
...
keep hhold rmo_new ruc_new strat_new
...
keep hhold stratum16comparable ruccomparable urbruralcomparable
...
merge 1:1 hhold using `stratum16comparable'	
keep if _m==3
drop _m
...
*****************************************************************************************************/
	...
	*keep only households with consumption information
	...
	#delimit ;
    order psu hhold term quarter hhwgt popwgt zilaid stratum16 ruc urbrural stratum16comparable ruccomparable urbruralcomparable 
	...
	rename (hhold zilaid) (hhid stratum)
